    ...
    override suspend fun reduce(viewIntent: MainViewIntents) {
        when (viewIntent) {
            ...
            is MainViewIntents.Sale -> {
                ProcessRepository.paymentData = ProcessRepository.paymentData.copy(actionType = SDKActionType.Sale)
                pushExternalIntent(SampleViewIntents.StartPaymentSDK)
            }
            //Tokenize
            is MainViewIntents.Tokenize -> {
                ProcessRepository.paymentData = ProcessRepository.paymentData.copy(actionType = SDKActionType.Tokenize)
                pushExternalIntent(SampleViewIntents.StartPaymentSDK)
            }
