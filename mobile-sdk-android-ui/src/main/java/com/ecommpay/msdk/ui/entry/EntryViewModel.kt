...
import com.ecommpay.msdk.ui.PaymentActivity.Companion.msdkSession
        ...
        initInteractor.execute(
            ...
            callback = object : InitDelegate {
                override fun onInitReceived(
                    ...
                    updateState(
                        DefaultViewStates.Content(
                            EntryViewData(
                                paymentMethodList = msdkSession.getPaymentMethods()
                                    ?.map { it.toViewData() } ?: emptyList(),
                ...
                override fun onError(code: ErrorCode, message: String) {
                    updateState(
                        DefaultViewStates.Content(
                            viewData = viewState.value?.viewData
                                ?: defaultViewData()
                        )
                    )
                    launchAction(
                        DefaultViewActions.ShowMessage(
                            MessageAlert(
                                "Ошибка сети",
                                message ?: "",
                                true
